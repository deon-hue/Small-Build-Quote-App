...
/**
 * Assembly Calculator — Gable roof structure (Roof → Roof Structure → "Pitched roof — gable ends"). Two
 * rafter faces meeting at a ridge, a wall plate at each of the two eaves walls, and ceiling joists tying the
 * rafter feet together across the full span. Sized like every Roof-phase calculator from the drawn shape's
 * bounding box, with the ridge length along one side and the overall span (eaves wall to eaves wall) along
 * the other — set which is which if the roof was drawn the other way round with the Swap button. The rafter
 * section is checked against the flat roof's own span chart (lib/flat-roof-joist-spans.ts), using this
 * roof's true, sloped rafter length as the span — the same span-chart module every flat-roof joist and the
 * mono-pitch roof's rafters already use. The gable end walls themselves are priced under External Walls, not
 * here. See lib/gable-roof.ts for the geometry.
 */
...
export default function AssemblyGableRoofDemo({ onClose, onSave, labourTrades = [], externalLengthMm, externalWidthMm }: Props) {
  ...
  const ridgeDepthMm = TIMBER_DEPTHS_MM.find(d => d >= rafterDepthMm) ?? 250
  const ridgeRate = timberRate('c24', ridgeDepthMm)
  ...
  const layers: AssemblyLayerDef[] = useMemo(() => {
    ...
    out.push({ id: 'rafters', name: `${rafterSectionLabel} rafters, both faces`, category: 'materials', source: 'fixed', fixedQty: g.rafterLm, unit: 'lm', unitCost: rafterRate, wastePct })
    out.push({ id: 'ridge', name: `Ridge board 47×${ridgeDepthMm} C24`, category: 'materials', source: 'fixed', fixedQty: g.ridgeLm, unit: 'lm', unitCost: ridgeRate, wastePct })
    out.push({ id: 'wall_plate', name: 'Wall plate 100×50 treated, both eaves walls', category: 'materials', source: 'fixed', fixedQty: g.wallPlateLm, unit: 'lm', unitCost: 2.80, wastePct })
    ...
    return out
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [g, rafterSectionLabel, rafterRate, ridgeDepthMm, ridgeRate, ceilingJoistDepthMm, ceilingJoistRate, wastePct])
  ...
  // Labour — the carpenter fixing the wall plates/ridge/rafters and the ceiling joists, following the roof
  // until edited by hand
  const labourSuggestions: LabourSuggestion[] = g ? suggestGableRoofLabour(g) : []
  ...
  const detail = detailOverride ?? (descInput ? describeGableRoof(descInput) : '')
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 16 }}>
        <div>
          {g && (<>
            ...
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 8, lineHeight: 1.5 }}>
              {g.rafterCount} rafters (both faces), true length {(g.rafterRunMm / 1000).toFixed(2)}m each ({g.rafterLm.toFixed(1)}lm total). Ridge {g.ridgeLm.toFixed(1)}m. Rise {(g.riseMm / 1000).toFixed(2)}m. Slope area {g.slopeAreaM2.toFixed(1)}m² (both faces). {g.ceilingJoistCount} ceiling joists.
            </div>
            <RafterSpanPanel
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end' }}>
            <div style={{ flex: 1 }}><PropRow label="Ridge length (mm)">{numInput(lengthMm, setLengthMm, 1)}</PropRow></div>
            <div style={{ flex: 1 }}><PropRow label="Overall span (mm)">{numInput(spanMm, setSpanMm, 1)}</PropRow></div>
            <button onClick={swapLengthSpan} title="Swap ridge length and span — if the roof was drawn the other way round"
          ...
          <CollapsibleSection title="Ceiling joists" borderColor="#bae6fd">
            ...
            <div style={{ fontSize: 11, color: '#64748b', marginTop: 4, lineHeight: 1.4 }}>Tie the rafter feet together across the full span, resting on both wall plates — one per rafter pair. Not checked against a span chart; a long span may need a binder or an engineer's design.</div>
          </CollapsibleSection>
...
// rafter length. Same panel style as the mono-pitch roof's own copy. ──
    ...
    <div style={{ marginTop: 8 }}>
      ...
      <details style={{ marginTop: 6 }}>
        ...
        <div style={{ fontSize: 10, color: '#64748b', marginTop: 4, lineHeight: 1.4 }}>
          Green reaches the {span}m true rafter length (half the span plus the eaves overhang, along the slope). Click a cell to use that section and those centres.
        </div>
...
// ── The roof's cross-section: two rafter faces meeting at a ridge, a wall plate at each eaves wall —
// the classic gable silhouette, drawn to the pitch so the shape reads straight off it. ──
function GableRoofSectionSvg({ g, eavesOverhangMm }: { g: GableRoofGeometry; eavesOverhangMm: number }) {
  const vbW = 380, vbH = 260
  const halfSpanPx = 130
  const k = halfSpanPx / (g.spanM * 500) // px per mm, half-span in px over half-span in mm
  ...
  return (
    <svg viewBox={`0 0 ${vbW} ${vbH}`} style={{ width: '100%', height: 220, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 6 }}>
      ...
      <text x={(xLeft + xRight) / 2} y={30} fontSize={10} fill="#57534e" textAnchor="middle">Ridge length {g.lengthM.toFixed(2)}m (into the page)</text>
      ...
      <g transform={`translate(20, ${vbH - 12})`}>
        ...
        <rect x={90} y={-6} width={14} height={6} fill="#0369a1" /><text x={109} y={0} fontSize={8} fill="#64748b">Ridge</text>
