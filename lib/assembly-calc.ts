...
/**
 * The shared shell. Identical for every future module — a layer names which Back Office
 * record it draws from and how its raw quantity is derived; this applies waste%, rounds to a
 * real purchase unit, and costs it. No markup is applied here — the quote's own markup does
...
export function calculateWallCost(input: WallInput, layers: AssemblyLayerDef[]): WallCostResult {
  ...
  const lines = layers.map(l => costLayer(l, geometry))
