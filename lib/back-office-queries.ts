...
export async function syncBackOfficeFromProduct(sb: SupabaseClient, userId: string): Promise<void> {
  ...
  console.log('[sync] phases in DB:', (dbPhases ?? []).length, '| with canonical_id:', phaseCanonToId.size, '| without:', unmappedPhases.length)
  console.log('[sync] sample unmapped phases:', unmappedPhases.slice(0, 3).map(p => p.name))
  if (unmappedPhases.length > 0) {
    for (const desired of desiredPhases) {
      ...
      if (match) {
        const { error: phaseHealErr } = await sb.from('bo_phases').update({ canonical_id: desired.canonical_id }).eq('id', match.id)
        if (phaseHealErr) console.error('[sync] phase heal error:', phaseHealErr.message)
        else {
          ...
          console.log('[sync] healed phase:', desired.name)
        }
      } else {
        console.log('[sync] no match for phase:', desired.name)
      }
    }
  }
  console.log('[sync] phaseCanonToId after heal:', phaseCanonToId.size)

  // ── Cleanup: remove phases that no longer belong ─────────────────────────────
  //    1. Old seeded phases with no canonical_id whose names aren't in TAKEOFF_PHASES
  //    2. Phases whose canonical_id has been removed from CANONICAL_PHASE_IDS
  //    Cascades to sub-phases and tasks automatically.
  const takeoffPhaseNames  = new Set<string>(TAKEOFF_PHASES as readonly string[])
  const validCanonicalIds  = new Set<string>(Object.values(CANONICAL_PHASE_IDS))
  const legacyPhases = (dbPhases ?? []).filter(p =>
    (!p.canonical_id && !takeoffPhaseNames.has(p.name)) ||
    (p.canonical_id  && !validCanonicalIds.has(p.canonical_id as string))
  )
